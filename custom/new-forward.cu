...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    if(M == 16){
        const unsigned int M_ = ceil(M / (float) TILE_WIDTH_2);

        dim3 blockDim(TILE_WIDTH_2, TILE_WIDTH_2, 1 );
        dim3 gridDim( ceil((W_out * H_out) / (float) ( TILE_WIDTH_2)), M_,B);

        conv_forward_kernel_2<<<gridDim,  blockDim >>>(device_y, device_x,  B, M , C, H, W, K);
    }else{
        const unsigned int W_grid  = ceil(W_out / (float) TILE_WIDTH);
